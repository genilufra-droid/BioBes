...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#e7ecef] font-sans text-[#263b4b]"><header className="fixed inset-x-0 top-0 z-50 border-b border-[#7d96a8] bg-gradient-to-b from-[#f8fafb] to-[#cbdbe6] shadow-[0_1px_2px_rgba(31,54,72,0.18)]"><div className="flex h-[34px] items-center px-2"><button onClick={() => navigate("/")} className="flex items-center gap-2 pr-4"><GenitMark /><span className="hidden text-xs font-bold text-[#244861] sm:block">Sistemi Genit Cloud</span></button><nav className="hidden h-full items-stretch lg:flex" aria-label="Menuja Alpha"><AlphaFileMenu onAction={handleFileAction} /><AlphaConfigMenu onAction={handleConfigAction} /><AlphaRegistrationMenu />{[["Raporte", () => navigate("/reports")], ["Instrumenta", () => navigate("/actions")], ["Ndihmë", () => navigate("/")]].map(([label, action]) => <button key={String(label)} onClick={action as () => void} className="border-x border-transparent px-2.5 text-[12px] font-medium text-[#253f53] hover:border-[#a1b2c0] hover:bg-white/75">{String(label)}</button>)}</nav><div className="ml-auto flex h-full items-center gap-1"><span className="hidden max-w-52 truncate px-2 text-[11px] text-[#405a6d] lg:block">{activeCompany?.name || "Kompania aktive"}</span><button onClick={() => setCompanyPanelOpen(true)} className="grid h-7 w-7 place-items-center rounded-sm hover:bg-white/75" aria-label="Ndrysho kompaninë"><Building2 className="h-4 w-4" /></button><button onClick={() => setMobileOpen(value => !value)} className="grid h-7 w-7 place-items-center rounded-sm hover:bg-white/75 lg:hidden" aria-label="Hap navigimin">{mobileOpen ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}</button><span className="hidden px-1 text-[11px] sm:block">{user.name || "Përdorues"}</span><button onClick={() => navigate("/change-password")} className="hidden px-2 text-[11px] text-[#4b5e6c] hover:underline sm:block">Fjalëkalimi</button><button onClick={() => logout()} className="px-2 text-[11px] text-[#4b5e6c] hover:underline">Dil</button></div></div><div className="flex h-[20px] items-center border-t border-white/60 bg-[#e6eef3] px-3 text-[10px] text-[#526a7b]"><span>Alpha Business / {location.startsWith("/settings") ? "Konfigurime" : activeModule.label}</span><span className="mx-2 text-[#a3b0b9]">|</span><span className="truncate">{location === "/" ? "Ambient kryesor" : location.startsWith("/settings") ? "Konfigurime dhe katalogë" : "Regjistrime dhe kërkesa"}</span><span className="ml-auto hidden sm:inline">Përdorues: {user.name || "Administrator"}</span></div></header>
    ...
    {mobileOpen && <div data-mobile-navigation className="fixed inset-x-0 top-[54px] z-40 max-h-[calc(100dvh-74px)] overflow-y-auto border-b border-[#aab7c2] bg-white p-2 shadow-lg lg:hidden"><p className="px-2 pb-1 text-[10px] font-bold uppercase tracking-wide text-[#587084]">Menuja kryesore</p><div className="grid grid-cols-2 border-y border-[#d0dbe2] py-1">{mobileQuickLinks.map(item => <button key={item.label} onClick={() => navigate(item.path)} className="px-3 py-2 text-left text-xs font-semibold text-[#244861] hover:bg-[#eaf3f9]">{item.label}</button>)}</div><p className="px-2 pb-1 pt-3 text-[10px] font-bold uppercase tracking-wide text-[#587084]">Modulet</p><div className="grid grid-cols-2">{alphaModuleItems.map(item => { const Icon = item.icon; return <button key={item.id} onClick={() => navigate(item.path)} className="flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-[#eaf3f9]"><Icon className="h-4 w-4" />{item.label}</button>; })}</div></div>}
  </div>;
}
