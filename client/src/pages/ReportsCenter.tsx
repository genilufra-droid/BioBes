...
import PurchaseReportInlineFilters from "@/components/PurchaseReportInlineFilters";
...
export default function ReportsCenter({ companyId }: { companyId: number }) {
  ...
  const isPurchaseWorkspace = moduleFilter === "Blerje";
  ...
  return <div className={"space-y-3 " + (isAlphaModuleWorkspace ? "text-[#242424]" : "")}>
    <section data-alpha-report-workspace className={"overflow-hidden border border-[#bdbdbd] bg-white " + (isAlphaModuleWorkspace ? "rounded-none shadow-none" : "rounded-xl shadow-sm")}>
    ...
    <section data-report-catalog className={"border border-[#bdbdbd] bg-white " + (isAlphaModuleWorkspace ? "rounded-none shadow-none" : "rounded-xl p-3 shadow-sm")}>
      {visibleReports.length === 0 ? <div className="border border-dashed border-[#bdbdbd] p-10 text-center text-sm text-[#666]">Nuk u gjet raport. Provoni një fjalë tjetër në Kerko.</div> : isPurchaseWorkspace ? <div className="space-y-0">
