...
import { getAlphaReportPresentation, resolveReportModuleForRoute } from "@/lib/reportPresentationPolicy";
...
export default function ReportsCenter({ companyId }: { companyId: number }) {
  ...
  useEffect(() => {
    ...
    const routeModule = resolveReportModuleForRoute(isModule(requestedModule) ? requestedModule : undefined, requestedReport);
    if (routeModule) setModuleFilter(routeModule);
    else setModuleFilter("Të gjitha");
    if (requestedReport && getAlphaReportPresentation(requestedReport)) {
  ...
  const selectedPresentation = getAlphaReportPresentation(selected.key);
  ...
  const isReferenceReport = selectedPresentation?.resultFormat === "alpha-reference";
