...
export default function ReportsCenter({ companyId }: { companyId: number }) {
  ...
  return <div className={`space-y-4 ${isAlphaModuleWorkspace ? "text-[#242424]" : ""}`}>
    <section className={`overflow-hidden border bg-white ${isAlphaModuleWorkspace ? "rounded-none border-[#c8c8c8] shadow-none" : "rounded-xl shadow-sm"}`}>
      <div className={`flex flex-col gap-3 border-b px-4 py-2.5 lg:flex-row lg:items-center lg:justify-between ${isAlphaModuleWorkspace ? "bg-white text-[#242424]" : "bg-[#714b67] text-white"}`}>
        <div className="flex items-center gap-2"><div className={`rounded-sm p-1.5 ${isAlphaModuleWorkspace ? "bg-[#0878c9] text-white" : "bg-white/15"}`}><BarChart3 className="h-4 w-4" /></div><div><h1 className="text-base font-semibold">{isAlphaModuleWorkspace ? `Raporte · ${alphaModuleLabels[moduleFilter as ReportModule] ?? moduleFilter}` : "Raportet"}</h1></div></div>
        <div className="flex items-center gap-2"><Badge className={`border-0 px-2.5 text-xs ${isAlphaModuleWorkspace ? "bg-[#f1f1f1] text-[#555]" : "bg-white/15 text-white"}`}>{isAlphaModuleWorkspace ? `${visibleReports.length} raporte` : `${REPORT_CATALOG.length} raporte · të ndara sipas modulit`}</Badge><Button size="sm" className={isAlphaModuleWorkspace ? "h-8 border border-[#bbb] bg-white text-[#444] hover:bg-[#f5f5f5]" : "bg-white text-[#714b67] hover:bg-white/90"} onClick={clearFilters}>Rivendos</Button></div>
      </div>
      {moduleFilter === "Të gjitha" && <nav aria-label="Nënmenuja Alpha e Raporteve" className="flex gap-1 overflow-x-auto border-b bg-[#0878c9] px-3 py-1.5 text-xs text-white">{alphaReportNavItems.map(item => <button type="button" key={item.label} disabled={!item.module} onClick={() => item.module && changeModule(item.module)} className={`shrink-0 border-r border-white/20 px-2 py-1 font-medium transition-colors ${item.module ? "hover:bg-white/15" : "cursor-not-allowed opacity-55"}`}>{item.label}</button>)}</nav>}
      <div className={`grid gap-3 p-3 ${isAlphaModuleWorkspace ? "bg-white lg:grid-cols-[minmax(260px,360px)_auto]" : "bg-[#f7f7f7] lg:grid-cols-[minmax(0,1fr)_auto_145px_145px]"}`}>
        <div className="relative"><Search className={`absolute left-3 top-3 h-4 w-4 ${isAlphaModuleWorkspace ? "text-[#0878c9]" : "text-[#714b67]"}`} /><Input id="reportSearch" value={query} onChange={event => setQuery(event.target.value)} className="h-9 border-[#bdbdbd] bg-white pl-9 text-sm" placeholder="Kerko" /></div>
        {!isAlphaModuleWorkspace && <><div className="flex h-10 items-center justify-end rounded-md border border-slate-300 bg-white px-3 text-xs text-slate-500" aria-label="Rrjedha e raporteve">Raporte → Moduli → Modeli</div><Input aria-label="Prej datës" type="date" value={dateFrom} onChange={event => setDateFrom(event.target.value)} className="h-10 border-slate-300 bg-white" /><Input aria-label="Deri më datën" type="date" value={dateTo} onChange={event => setDateTo(event.target.value)} className="h-10 border-slate-300 bg-white" /></>}
      </div>
    </section>

    <section data-report-catalog className="rounded-xl border bg-white p-3 shadow-sm">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-sm font-semibold uppercase tracking-wide text-slate-700">Modelet e Raporteve</h2><p className="text-xs text-slate-500">{moduleFilter === "Të gjitha" ? "Zgjidhni një modul për të hapur listën e modeleve, si në Alpha Web." : `Modelet e ${alphaModuleLabels[moduleFilter as ReportModule] ?? moduleFilter} · zgjidhni një dokument për filtrat.`}</p></div><div className="flex items-center gap-2"><span className="text-xs text-slate-500">{moduleFilter === "Të gjitha" ? `${visibleReports.length} / ${REPORT_CATALOG.length}` : `${visibleReports.length} raporte`}</span><Button type="button" size="sm" variant="outline" onClick={() => changeModule("Të gjitha")} disabled={moduleFilter === "Të gjitha"}>Kryesore</Button></div></div>
      {visibleReports.length === 0 ? <div className="rounded-lg border border-dashed p-10 text-center text-sm text-slate-500">Nuk u gjet raport. Provoni një fjalë tjetër në Kerko.</div> : <div className="space-y-4">
        <details open className={`rounded-sm border border-slate-200 bg-white ${moduleFilter !== "Të gjitha" ? "hidden" : ""}`}>
        ...
        {moduleFilter !== "Të gjitha" && groupedReports.length > 0 && <div data-report-model-list className="rounded-sm border border-slate-200 bg-[#f7f7f7] p-3"><div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-slate-300 pb-2"><div className="flex items-center gap-2"><Badge variant="secondary" className={moduleColors[moduleFilter as ReportModule]}>{alphaModuleLabels[moduleFilter as ReportModule] ?? moduleFilter}</Badge><span className="text-xs text-slate-500">Modelet e raportit</span></div><div className="flex flex-wrap items-center gap-1"><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => toast.info("Modelet e Raporteve vijnë nga katalogu i kompanisë aktive.")}>Ruaj</Button><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => openReport(selected)}>Shiko</Button><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => toast.info("Shtimi i modeleve të reja menaxhohet nga katalogu i Raporteve.")}>Shto</Button><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => toast.info("Zgjidhni një model nga lista për ta klonuar.")}>Klono</Button><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => toast.info("Konfigurimi i modelit ruhet sipas autorizimeve të kompanisë aktive.")}>Modifiko</Button><Button type="button" size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={() => toast.info("Fshirja e modeleve nuk lejohet nga katalogu i Raporteve.")}>Fshi</Button></div></div><div className="mb-3 flex flex-wrap items-center gap-1 border-b border-slate-300"><Button type="button" size="sm" variant="ghost" className={`rounded-none border-b-2 px-3 text-xs ${reportWorkspaceTab === "Të përgjithshme" ? "border-[#714b67] text-[#714b67]" : "border-transparent text-slate-500"}`} onClick={() => setReportWorkspaceTab("Të përgjithshme")}>Të përgjithshme</Button><Button type="button" size="sm" variant="ghost" className={`rounded-none border-b-2 px-3 text-xs ${reportWorkspaceTab === "Konfigurimi" ? "border-[#714b67] text-[#714b67]" : "border-transparent text-slate-500"}`} onClick={() => setReportWorkspaceTab("Konfigurimi")}>Konfigurimi</Button><Button type="button" size="sm" variant="ghost" className={`rounded-none border-b-2 px-3 text-xs ${reportWorkspaceTab === "Raporti" ? "border-[#714b67] text-[#714b67]" : "border-transparent text-slate-500"}`} onClick={() => setReportWorkspaceTab("Raporti")}>Raporti</Button></div>{reportWorkspaceTab === "Raporti" && <div className="mb-3 space-y-3 rounded-sm border border-slate-300 bg-white p-3"><div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2"><span className="text-xs font-semibold text-slate-700">Filtrat</span><select aria-label="Tipi i grafikut" className="h-8 border border-slate-300 bg-white px-2 text-xs"><option>Line</option><option>Bar</option><option>Pie</option></select><Button type="button" size="sm" variant="outline" className="h-8 px-2 text-xs" onClick={() => toast.info("Grafiku do të përdorë kolonat numerike të raportit aktiv.")}>Me Grafik</Button><Button type="button" size="sm" variant="outline" className="h-8 px-2 text-xs" onClick={() => toast.info("Pamja pa grafik u zgjodh për raportin aktiv.")}>Pa Grafik</Button><Button type="button" size="sm" variant="outline" className="h-8 px-2 text-xs" onClick={() => toast.info("Filter-i i ri do të ruhet te konfigurimi i modelit aktiv.")}>Krijo Filter</Button><Button type="button" size="sm" variant="ghost" className="h-8 px-2 text-xs" onClick={() => toast.info("Nuk ka ndryshime të reja për Undo.")}>Undo</Button><Button type="button" size="sm" variant="ghost" className="h-8 px-2 text-xs" onClick={() => toast.info("Nuk ka ndryshime të reja për Redo.")}>Redo</Button></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5 text-xs text-slate-600"><label className="flex items-center gap-2"><input type="checkbox" defaultChecked />Totali për rreshtat e grupuar</label><label className="flex items-center gap-2"><input type="checkbox" />Subtotali për rreshtat e grupuar</label><label className="flex items-center gap-2"><input type="checkbox" />Totali për kolonat e grupuara</label><label className="flex items-center gap-2"><input type="checkbox" />Subtotali për kolonat e grupuara</label><label className="flex items-center gap-2"><input type="checkbox" />Hiq vlera zero</label></div><div className="grid gap-2 md:grid-cols-2"><div className="rounded-sm border border-slate-200"><p className="border-b bg-slate-50 px-2 py-1 text-[11px] font-semibold uppercase">Drop Filter Fields Here</p><div className="min-h-10 p-2 text-xs text-slate-500">{columns.filter(column => /klient|furnitor|kategori|magazin|dat|monedh|status/i.test(column)).slice(0, 4).join(" · ") || "—"}</div></div><div className="rounded-sm border border-slate-200"><p className="border-b bg-slate-50 px-2 py-1 text-[11px] font-semibold uppercase">Drop Column Fields Here</p><div className="min-h-10 p-2 text-xs text-slate-500">{columns.filter(column => /kategori|grup|muaj|vit/i.test(column)).slice(0, 4).join(" · ") || "—"}</div></div><div className="rounded-sm border border-slate-200"><p className="border-b bg-slate-50 px-2 py-1 text-[11px] font-semibold uppercase">Drop Row Fields Here</p><div className="min-h-10 p-2 text-xs text-slate-500">{columns.filter(column => !/vleft|vler|sasi|cmim|total|detyrim|debi|kredi/i.test(column)).slice(0, 4).join(" · ") || columns.slice(0, 4).join(" · ") || "—"}</div></div><div className="rounded-sm border border-slate-200"><p className="border-b bg-slate-50 px-2 py-1 text-[11px] font-semibold uppercase">Drop Data Items Here</p><div className="min-h-10 p-2 text-xs text-slate-500">{columns.filter(column => /vleft|vler|sasi|cmim|total|detyrim|debi|kredi/i.test(column)).slice(0, 4).join(" · ") || columns.slice(0, 2).join(" · ") || "—"}</div></div></div></div>}<div className="mb-3 flex items-center gap-2"><Input aria-label="Kerko modelin e raportit" placeholder="Kerko..." value={query} onChange={event => setQuery(event.target.value)} className="h-8 max-w-xs bg-white text-xs" /><span className="text-xs text-slate-500">Tërhiqni këtu kolonat për t’i grupuar</span></div><div className="space-y-2">{groupedReports.map((moduleGroup, groupIndex) => <details key={moduleGroup.module} open className="rounded-sm border border-slate-200 bg-white"><summary className="cursor-pointer list-none border-b border-slate-200 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-700">{moduleFilter === "Blerje" ? (groupIndex === 0 ? "Kryesore" : "Te Tjera 2") : (moduleGroup.groups.length > 0 ? moduleGroup.groups.map(group => group.group).join(" · ") : "Modele")}</summary><div className={`grid gap-1.5 p-2 ${isAlphaModuleWorkspace ? "grid-cols-2 sm:grid-cols-4 lg:grid-cols-7" : "sm:grid-cols-2 xl:grid-cols-4"}`}>{moduleGroup.reports.map(item => <button type="button" key={item.key} onClick={() => openReport(item)} className={`group flex min-h-16 items-center justify-between rounded-sm border-l-4 p-3 text-left transition-all ${selectedKey === item.key ? "border-l-[#714b67] border-y-[#e2d6e0] border-r-[#e2d6e0] bg-[#fbf7fa] shadow-sm" : "border-l-slate-300 border-y-slate-200 border-r-slate-200 bg-white hover:border-l-[#714b67] hover:bg-slate-50"}`}><h3 className="text-sm font-semibold uppercase tracking-wide text-slate-800">{item.title.toLocaleUpperCase("sq-AL")}</h3><ArrowUpRight className="h-4 w-4 shrink-0 text-[#714b67]" /></button>)}</div></details>)}</div></div>}
      </div>}
    </section>
