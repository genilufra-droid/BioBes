...
export default function ReportsCenter({ companyId }: { companyId: number }) {
  ...
  const [moduleFilter, setModuleFilter] = useState<ReportModule | "Të gjitha">(() => {
    ...
    return requestedReport?.module ?? "Blerje";
  });
  ...
  useEffect(() => {
    ...
    else if (requestedReport) setModuleFilter(REPORT_CATALOG.find(item => item.key === requestedReport)?.module ?? "Blerje");
  ...
  const visibleReports = useMemo(() => {
    ...
    return REPORT_CATALOG.filter(item => {
      const matchesModule = moduleFilter === "Të gjitha" || item.module === moduleFilter;
      const searchable = `${item.title} ${item.description} ${item.module} ${item.group} ${item.key}`.toLocaleLowerCase("sq-AL");
      return matchesModule && (!search || searchable.includes(search));
    });
  }, [moduleFilter, query]);
  ...
  const activeFilterEntries = useMemo(() => activeReportFilters({ [selected.key === "inventory_product_card_pdf" ? "Dokumenti burimor" : "Furnitor / Klient"]: partnerFilter, "Nr. dokumenti": effectiveDocumentFilter, [selected.key === "inventory_product_card_pdf" ? "Artikull" : "Kategori / Artikull"]: categoryFilter, Status: statusFilter, Monedha: currencyFilter, "Lloj dokumenti": documentTypeFilter, [selected.key === "inventory_product_card_pdf" ? "Magazinë" : "Magazina"]: warehouseFilter, Njësia: unitFilter, "Shuma minimale": amountMin, "Shuma maksimale": amountMax, "Data nga": dateFrom, "Data deri": dateTo, "Kërkimi në tabelë": tableSearch }), [partnerFilter, documentFilter, categoryFilter, statusFilter, currencyFilter, documentTypeFilter, warehouseFilter, unitFilter, amountMin, amountMax, dateFrom, dateTo, tableSearch]);
  const activeFilterMeta = useMemo(() => Object.fromEntries(activeFilterEntries), [activeFilterEntries]);
  const isReferencePdf = selected.key.endsWith("_pdf");
  ...
  const changeModule = (nextModule: ReportModule | "Të gjitha") => {
    ...
    setLocation(reportUrl(nextModule, nextReport.key));
    if (nextModule !== "Të gjitha") setIsReportOpen(true);
  };
  ...
  const referenceTitle = isReferencePdf ? getReferenceTitle(selected.key, selected.title) : selected.title;
  ...
  const exportExcel = () => exportToExcel(rows.map(row => Object.fromEntries(Object.entries(row).filter(([key]) => !key.startsWith("__")))), `${selected.key}_${new Date().toISOString().slice(0, 10)}`, referenceTitle, isReferencePdf ? columns.map(column => ({ key: column, label: getReferenceColumnLabel(selected.key, column) })) : columns, { title: referenceTitle, period: `${dateFrom || "Fillimi"} — ${dateTo || "Sot"}`, landscape: columns.length > 8, reference: isReferencePdf, includeTotals: true, headerColor: isReferencePdf ? "FF714B67" : undefined, titleColor: isReferencePdf ? "FF714B67" : undefined, referenceKey: isReferencePdf ? selected.key : undefined });
  const exportPdf = () => exportToPDF(rows, `${selected.key}_${new Date().toISOString().slice(0, 10)}`, referenceTitle, columns.map(column => ({ key: column, label: isReferencePdf ? getReferenceColumnLabel(selected.key, column) : column })) as { key: keyof Record<string, unknown>; label: string }[], { landscape: columns.length > 8, reference: isReferencePdf, includeTotals: true, referenceKey: isReferencePdf ? selected.key : undefined, period: `${dateFrom || "Fillimi"} — ${dateTo || "Sot"}`, meta: reportMeta, headerColor: [230, 229, 181], headerTextColor: [37, 37, 31], titleColor: [37, 37, 31], fontSize: columns.length > 10 ? 6 : 8 });
  ...
  const printPreview = () => {
    ...
    const printGroups = isReferencePdf ? getReferenceGroups(selected.key, columns) : [];
    const printHeaderRows = isReferencePdf ? `${printGroups.length > 0 ? `<tr>${printGroups.map(group => `<th colspan="${group.columns.length}" class="group">${group.label}</th>`).join("")}</tr>` : ""}<tr>${columns.map(column => `<th>${getReferenceColumnLabel(selected.key, column)}</th>`).join("")}</tr>` : `<tr>${columns.map(column => `<th>${column}</th>`).join("")}</tr>`;
    const printTitle = isReferencePdf ? `<div class="reference-head"><span>${new Date().getFullYear()}</span><span>${period}</span><strong>${referenceTitle}</strong></div>` : `<h1>${selected.title}</h1><p class="meta">${selected.module} · Periudha: ${period}</p>`;
    const printMetrics = isReferencePdf ? "" : `<div class="metrics">${(report?.metrics ?? []).map(item => `<div class="metric"><small>${item.label}</small><b>${item.value.toLocaleString("sq-AL")}</b></div>`).join("")}</div>`;
    const printTotalCells = columns.map((column, index) => { const total = sumNumericColumn(rows, column); return `<td>${index === 0 ? (isReferencePdf ? getReferenceTotalLabel(selected.key) : "TOTALI") : total === null ? "" : cellValue(total)}</td>`; }).join("");
    ...
    popup.document.write(`<!doctype html><html><head><title>${referenceTitle}</title><style>@page{size:A4 ${columns.length > 8 ? "landscape" : "portrait"};margin:8mm}body{font-family:Arial,sans-serif;color:#252525;padding:32px;background:#f3f4f6}.report{background:#fff;padding:24px;border:1px solid #d8d8d8}h1{color:#714b67;margin:0;text-align:center;font-size:20px}.reference-head{display:grid;grid-template-columns:1fr 2fr 1fr;align-items:start;border-bottom:1px solid #4a4a36;padding-bottom:10px;color:#25251f;font-size:10px}.reference-head span:last-of-type{text-align:right}.reference-head strong{grid-column:1 / -1;grid-row:1;text-align:center;text-transform:uppercase;font-size:16px}.meta{color:#666;margin:8px 0 22px;text-align:center}.metrics{display:flex;gap:12px;margin-bottom:20px}.metric{border:1px solid #d8c5d2;padding:12px;min-width:120px;border-radius:3px;background:#fcf8fb}.metric b{display:block;font-size:20px;color:#714b67}table{width:100%;border-collapse:collapse;font-size:${columns.length > 10 ? "8px" : "12px"}}.active-filters{margin:10px 0;padding:7px 9px;border:1px solid #e2c98b;background:#fff8e8;color:#6b4b16;font-size:10px}thead{display:table-header-group}tfoot{display:table-footer-group}tr{break-inside:avoid;page-break-inside:avoid}th{background:${isReferencePdf ? "#e6e5b5" : "#714b67"};color:${isReferencePdf ? "#25251f" : "#fff"};text-align:left}th.group{background:${isReferencePdf ? "#f1f0c8" : "#e9dce7"};color:${isReferencePdf ? "#25251f" : "#55394f"};text-align:center;text-transform:uppercase;font-size:10px}th,td{border:1px solid ${isReferencePdf ? "#8a8a63" : "#cfcfcf"};padding:7px}.source-link{color:#714b67;font-weight:700;text-decoration:underline}.total-row{font-weight:700;background:${isReferencePdf ? "#f1f0c8" : "#fcf8fb"}}tr:nth-child(even){background:${isReferencePdf ? "#fffef1" : "#fafafa"}}footer{display:flex;justify-content:space-between;border-top:2px solid ${isReferencePdf ? "#4a4a36" : "#714b67"};margin-top:18px;padding-top:8px;color:#666;font-size:9px;text-transform:uppercase}@media print{body{padding:0;background:#fff}.report{border:0;padding:0}}</style></head><body><div class="report">${printTitle}${activeFilterEntries.length > 0 ? `<div class="active-filters"><strong>Filtra aktive:</strong> ${activeFilterEntries.map(([label, value]) => `${label}: ${value}`).join(" · ")}</div>` : ""}${printMetrics}<table><thead>${printHeaderRows}</thead><tbody>${printRows}</tbody><tfoot><tr class="total-row">${printTotalCells}</tr></tfoot></table><footer><span>${isReferencePdf ? getReferenceTotalLabel(selected.key) : "Totali i raportit"}</span><span>Sistemi Genit Cloud · Dokumenti mund të ketë disa faqe</span></footer></div><script>window.onload=()=>window.print()</script></body></html>`);
    popup.document.close();
  };
  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
      ...
      <div className="grid gap-3 bg-[#f7f7f7] p-3 lg:grid-cols-[minmax(0,1fr)_auto_145px_145px]">
        <div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-[#714b67]" /><Input id="reportSearch" value={query} onChange={event => setQuery(event.target.value)} className="h-10 border-slate-300 bg-white pl-9" placeholder="Live Search: kërko raport, modul ose grup..." /></div>
        <div className="flex h-10 items-center gap-1 overflow-x-auto rounded-md border border-slate-300 bg-white p-1" aria-label="Modulet e raporteve">{reportModules.map(module => <button type="button" key={module} onClick={() => changeModule(module)} className={`shrink-0 rounded px-2.5 py-1 text-xs font-medium transition-colors ${moduleFilter === module ? "bg-[#714b67] text-white" : "text-slate-600 hover:bg-slate-100"}`}>{module}</button>)}</div>
